...
const listproperty = async (req, res) => {
  console.log("listproperty controller reached"); // Added log
  try {
...
const singleproperty = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ message: "Invalid property ID format", success: false });
    }
    ...
    if (error.name === "CastError") {
      return res
        .status(400)
        .json({
          message: `Invalid property ID format: ${error.value} is not a valid ObjectId.`,
          success: false,
        });
    }
    res.status(500).json({ message: "Server Error", success: false });
